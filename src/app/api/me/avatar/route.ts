  ...
  try {
    ...
    if (!ALLOWED_AVATAR_TYPES.has(file.type)) {
      return NextResponse.json(
        { ok: false, error: "Format non supporté. Utilisez JPG, PNG, WebP, GIF ou AVIF." },
        { status: 400 }
      );
    }
    if (file.size > MAX_AVATAR_UPLOAD_BYTES) {
      return NextResponse.json(
        { ok: false, error: "Image trop volumineuse (maximum 20MB)." },
        { status: 413 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: "Impossible de traiter cette image. Essayez une image JPG, PNG ou WebP." },
      { status: 500 }
    );
  }
}
