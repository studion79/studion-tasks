...
export function ProjectTimelineView({ project }: { project: ProjectWithRelations }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Legend */}
      <div className="flex items-center gap-4 px-4 py-2 bg-gray-50 border-b border-gray-100 flex-shrink-0 text-xs text-gray-500">
        <span className="font-medium text-gray-700">Échéancier par responsable</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-shrink-0 border-r border-gray-200 bg-white" style={{ width: LABEL_W }}>
          {/* Header spacer */}
          ...
          {ownerGroups.map(([owner, tasks], idx) => {
            ...
            return (
              <div
                key={owner}
                className="flex items-start gap-2 px-3 py-2 border-b border-gray-100"
                style={{ height: rowH }}
              >
                <div className="w-6 h-6 rounded-full flex-shrink-0 mt-1 overflow-hidden">
                  {!isNoOwner && memberAvatars[owner] ? (
                    ...
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isNoOwner ? "bg-gray-200" : colorSet.bg}`}>
                      <span className={`text-[9px] font-bold ${isNoOwner ? "text-gray-500" : colorSet.text}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-gray-700 truncate leading-tight mt-1">
          ...
          <div style={{ width: totalDays * DAY_PX, minWidth: "100%" }}>
            ...
            <div className="flex border-b border-gray-200 bg-gray-50 sticky top-0 z-10" style={{ height: 28 }}>
              {weekGroups.map((wg) => (
                <div
                  key={wg.offset}
                  className="flex-shrink-0 flex items-center justify-center border-r border-gray-200 text-[10px] font-medium text-gray-500 overflow-hidden"
            ...
            {/* ---- Day header row ---- */}
            <div className="flex border-b border-gray-200 bg-white sticky top-7 z-10" style={{ height: 28 }}>
              {Array.from({ length: totalDays }).map((_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex-shrink-0 flex flex-col items-center justify-center border-r border-gray-100 ${weekend ? "bg-gray-50" : ""} ${isToday ? "bg-rose-50" : ""}`}
                    style={{ width: DAY_PX }}
                  >
                    <span className={`text-[9px] font-medium leading-none ${isToday ? "text-rose-600" : weekend ? "text-gray-400" : "text-gray-500"}`}>
                      {d.getDate()}
                    </span>
                    <span className={`text-[8px] leading-none mt-0.5 ${isToday ? "text-rose-400" : weekend ? "text-gray-300" : "text-gray-300"}`}>
