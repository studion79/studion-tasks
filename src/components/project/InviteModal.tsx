...
export function InviteModal({
  ...
  const handleInviteGroup = (group: UserGroupRow) => {
    ...
    startTransition(async () => {
      ...
      const msg = sent === 0 ? t("invite.alreadyMembers") : `${sent} ${t("invite.invitation")}${sent > 1 ? "s" : ""} ${t("invite.sent")}${sent > 1 ? "s" : ""}`;
  ...
  const handleInvite = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setSuccess(`${t("invite.sentTo")} ${email.trim()}`);
        setEmail("");
        ...
        if (raw.includes("FORBIDDEN_INVITE_PERSONAL_PROJECT")) {
          setError(t("project.personalNoMembers"));
        } else {
      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none">
        <div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl p-4 sm:p-6 w-full sm:max-w-sm pointer-events-auto h-[92dvh] sm:h-auto sm:max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4 sticky top-0 bg-white dark:bg-gray-800 z-10 py-1">
            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-50">{t("invite.projectMembers")}</h2>
                ...
                <div key={m.userId} className="flex items-center gap-2.5 group">
                  ...
                  {canManageMembers ? (
                    (() => {
                      ...
                      const roleTitle = isLastAdmin
                        ? t("invite.cannotDemoteLastAdmin")
                        : m.role === "ADMIN"
                          ? t("invite.demoteToMember")
                          : t("invite.promoteAdmin");
                      return (
                    <button
                      onClick={() => {
                    ...
                    >
                      {m.role === "ADMIN" ? "Admin" : t("me.member")}
                    </button>
                      );
                    })()
                  ) : (
                  ...
                  {canManageMembers && (
                    <button
                      ...
                      aria-label={t("invite.removeMember")}
          ...
          {pendingInvitations.length > 0 && (
            <div className="mb-4">
              <p className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{t("invite.pending")}</p>
                ...
                {pendingInvitations.map((inv) => (
                  <div key={inv.id} className="flex items-center gap-2.5 group py-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400 dark:text-gray-500">{t("invite.invitationSent")}</p>
                    </div>
                    {canManageMembers && (
                      <button
                        onClick={() => handleCancelInvitation(inv.id)}
                        aria-label={t("invite.cancelInvitationAria")}
          ...
          {userGroups.length > 0 && (
            <div className="mb-4 border-t border-gray-100 dark:border-gray-700 pt-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t("invite.groups")}</p>
                {canManageMembers && (
                  <button
                    ...
                    <div key={g.id} className="flex items-center justify-between gap-2 py-1">
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-gray-400 dark:text-gray-500">
                          {emails.length} {t("me.member")}{emails.length > 1 ? "s" : ""}
                        </p>
                      </div>
                      {canInvite && (
                        status === "loading" ? (
                          <span className="text-[11px] text-gray-400">{t("common.sending")}</span>
                        ) : status ? (
                          <span className={`text-[11px] ${status === t("invite.alreadyMembers") ? "text-gray-400" : "text-green-600"}`}>{status}</span>
                        ) : (
                          <button
          ...
          {canManageMembers && userGroups.length === 0 && (
            <div className="mb-4 border-t border-gray-100 dark:border-gray-700 pt-4">
              ...
              >
                ...
                {t("invite.configureInvitationGroups")}
              </button>
            </div>
          )}
          ...
          {canInvite && (
            <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
              ...
              <form onSubmit={handleInvite} className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("invite.inviteByEmail")}</label>
                  <input
                    type="email"
                    value={email}
                ...
                >
                  {isPending ? t("common.sending") : t("invite.sendInvitation")}
                </button>
              </form>
