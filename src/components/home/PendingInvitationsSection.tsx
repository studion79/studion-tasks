    ...
    <div className="mb-8">
      ...
      <div className="space-y-3">
        {list.map((inv) => (
          <div
            key={inv.id}
            className="bg-white border border-indigo-100 rounded-xl px-4 py-3.5 flex items-center gap-4 shadow-sm"
          >
            {/* Icon */}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">{inv.project.name}</p>
              {inv.project.description && (
                <p className="text-xs text-gray-400 truncate">{inv.project.description}</p>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                disabled={isPending}
                className="text-xs text-gray-400 hover:text-gray-600 px-2.5 py-1.5 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer disabled:opacity-50"
              >
                Refuser
              </button>
              <button
